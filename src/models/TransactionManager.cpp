...
const OneTimeTransfer &TransactionManager::createTransaction(
        const QDateTime &date, const Account &to, const Account &from, const CASH_AMOUNT_T &amount
) {
    if (dynamic_cast<const CheckingAccount *>(&from)) {
        ...
        throw TransactionException("Wrong account type");
    }
}

const RegularTransfer &
TransactionManager::createTransaction(
        const QDateTime &date, const Account &to, const Account &from, const CASH_AMOUNT_T &amount, int reg
) {
    if (dynamic_cast<const CheckingAccount *>(&from)) {
...
const CashTransaction &TransactionManager::createTransaction(
        const QDateTime &date, const Account &from, const ACCOUNT_BALANCE_AMOUNT_T &amount, bool isWithdrawal
) {
    if (dynamic_cast<const CheckingAccount *>(&from)) {
...
TransactionManager::createTransaction(
        const QDateTime &date, const Account &from, const CARD_NUMBER_T &card_n,
        AccountManaging::ValueChanged valueType, unsigned __int64 oldValue, unsigned __int64 newValue
) {
