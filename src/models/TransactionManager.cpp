...
CashTransaction
TransactionManager::createTransaction(const QDateTime &date, const Account &from, const ACCOUNT_BALANCE_AMOUNT_T amount, const bool isWithdrawal) {
    if (const auto *t = dynamic_cast<const CheckingAccount *>(&from)) {
...
TransactionManager::createTransaction(
        const QDateTime &date, const Account &from, const CARD_NUMBER_T card_n, const AccountManaging::ValueChanged valueType,
        const VAL oldValue, const VAL newValue
) {
